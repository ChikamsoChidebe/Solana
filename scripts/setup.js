...
async function setupCarbonChain() {
    ...
    try {
        ...
        console.log("SUCCESS: Wallet loaded:", wallet.publicKey.toString());
    } catch (error) {
        console.error("ERROR: Failed to load wallet. Make sure you have a Solana keypair configured.");
        process.exit(1);
    }
    ...
    if (balance < 0.1 * LAMPORTS_PER_SOL) {
        ...
        try {
            ...
            console.log("SUCCESS: Airdrop successful");
        } catch (error) {
            console.error("ERROR: Airdrop failed:", error.message);
        }
    }

    console.log("\nSetting up CarbonChain infrastructure...");
    ...
    try {
        ...
        if (!marketplaceAccount) {
            ...
            console.log("   SUCCESS: Marketplace initialized at:", marketplacePda.toString());
        } else {
            console.log("   SUCCESS: Marketplace already exists at:", marketplacePda.toString());
        }
        ...
        if (!registryAccount) {
            ...
            console.log("   SUCCESS: Registry initialized at:", registryPda.toString());
        } else {
            console.log("   SUCCESS: Registry already exists at:", registryPda.toString());
        }
        ...
        if (!verifierAccount) {
            ...
            console.log("   SUCCESS: Verifier initialized at:", verifierPda.toString());
        } else {
            console.log("   SUCCESS: Verifier already exists at:", verifierPda.toString());
        }

        // 4. Check for existing projects
        console.log("\nChecking for existing carbon projects...");
